make_swc_and_transpiration_plots_based_on_raw_data <- function() {
    ...
    #### plotting
    p1 <- ggplot(pilDF, aes(x=Day, y=SWC.mean, group=Trt)) +
        geom_errorbar(aes(col=Trt, x=Day, 
                          ymin=SWC.mean-SWC.se, ymax=SWC.mean+SWC.se),
                      width=0.2)+
    ...
    p2 <- ggplot(popDF, aes(x=Day, y=SWC.mean, group=Trt)) +
        geom_errorbar(aes(col=Trt, x=Day, 
                          ymin=SWC.mean-SWC.se, ymax=SWC.mean+SWC.se),
                      width=0.2)+
        geom_point(aes(col=Trt, fill=Trt, pch=Trt), size=2)+
    ...
    p3 <- ggplot(pilDF, aes(x=Day, y=transp_plant.mean, group=Trt)) +
        geom_errorbar(aes(col=Trt, x=Day, 
    ...
    p4 <- ggplot(popDF, aes(x=Day, y=transp_plant.mean, group=Trt)) +
        geom_errorbar(aes(col=Trt, x=Day, 
                          ymin=transp_plant.mean-transp_plant.se, 
                          ymax=transp_plant.mean+transp_plant.se),
                      width=0.2)+
    ...
    pdf(paste0(outdir, "F3.swc_and_transpiration_based_on_raw_data.pdf"), width=8, height=8)
