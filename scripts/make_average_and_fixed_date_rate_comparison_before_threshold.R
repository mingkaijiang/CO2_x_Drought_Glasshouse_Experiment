make_average_and_fixed_date_rate_comparison_before_threshold <- function() {
    ...
    myDF1 <- summaryBy(psiPD+psiMD+transp_plant+E_psi+SWC+Aearly+Alate+gsearly+gslate+Adaily+gsdaily~Replicate+Trt+CO2+H2O+Glasshouse,
                       FUN=mean, data=myDF1, keep.names=T)
    
    myDF2 <- summaryBy(psiPD+psiMD+transp_plant+E_psi+SWC+Aearly+Alate+gsearly+gslate+Adaily+gsdaily~Replicate+Trt+CO2+H2O+Glasshouse,
                       FUN=mean, data=myDF2, keep.names=T)
