make_leaf_water_potential_plots_based_on_raw_data <- function() {
    ...
    #### plotting
    p1 <- ggplot(pilDF, aes(x=Day, y=psiPD.mean, group=Trt)) +
        geom_errorbar(aes(col=Trt, x=Day, 
                          ymin=psiPD.mean-psiPD.se, ymax=psiPD.mean+psiPD.se),
                      width=0.2)+
    ...
    p2 <- ggplot(popDF, aes(x=Day, y=psiPD.mean, group=Trt)) +
        geom_errorbar(aes(col=Trt, x=Day, 
                          ymin=psiPD.mean-psiPD.se, ymax=psiPD.mean+psiPD.se),
                      width=0.6)+
        geom_point(aes(col=Trt, fill=Trt, pch=Trt), size=2)+
    ...
    p3 <- ggplot(pilDF, aes(x=Day, y=psiMD.mean, group=Trt)) +
        geom_errorbar(aes(col=Trt, x=Day, 
                          ymin=psiMD.mean-psiMD.se, ymax=psiMD.mean+psiMD.se),
                      width=0.2)+
        geom_point(aes(col=Trt, fill=Trt, pch=Trt), size=2)+
        geom_line(aes(col=Trt, lty=Trt))+
        theme_linedraw() +
    ...
    p4 <- ggplot(popDF, aes(x=Day, y=psiMD.mean, group=Trt)) +
        geom_errorbar(aes(col=Trt, x=Day, 
                          ymin=psiMD.mean-psiMD.se, ymax=psiMD.mean+psiMD.se),
                      width=0.6)+
        geom_point(aes(col=Trt, fill=Trt, pch=Trt), size=2)+
    ...
    pdf(paste0(outdir, "F5.leaf_water_potentials_based_on_raw_data.pdf"), width=8, height=8)
