make_whole_plant_hydraulic_conductance_plot <- function() {
    ...
    pdf("output/F7.whole_plant_hydraulics_plot.pdf", width=8, height=6)
    #A
    ...
    par(las=1)
    with(PILPhys,plot(E_psi[Trt=="PILAD"]~Day[Trt=="PILAD"],col="red",pch=1,cex.axis=1.25,
    ...
    with(PILPhys,points(E_psi[Trt=="PILAND"]~Day[Trt=="PILAND"],col="blue",pch=1,cex=1.25))
    with(PILPhys,points(E_psi[Trt=="PILED"]~Day[Trt=="PILED"],col="red",pch=16,cex=1.25))
    with(PILPhys,points(E_psi[Trt=="PILEND"]~Day[Trt=="PILEND"],col="blue",pch=16,cex=1.25))
    par(las=3)
    ...
    par(mar=c(2, 0.5, 4, 1))
    par(las=1)
    with(POPPhys,plot(E_psi[Trt=="POPAD"]~Day[Trt=="POPAD"],col="red",pch=1,cex.axis=1.25,
                      ylim=range(0,7),xlim=range(0,1.05*max(Day)),
                      ylab="",yaxt="n",xaxt="n",cex.lab=1,cex=1.25))
    axis(1,labels=TRUE,tck=-0.02,cex.lab=1,cex.axis=1.25,cex=1.25)
    with(POPPhys,points(E_psi[Trt=="POPAND"]~Day[Trt=="POPAND"],col="blue",pch=1,cex=1.25))
    with(POPPhys,points(E_psi[Trt=="POPED"]~Day[Trt=="POPED"],col="red",pch=16,cex=1.25))
    with(POPPhys,points(E_psi[Trt=="POPEND"]~Day[Trt=="POPEND"],col="blue",pch=16,cex=1.25))
    par(las=3)
